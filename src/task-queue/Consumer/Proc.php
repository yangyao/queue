...
namespace Yangyao\Queue\Consumer;
use Yangyao\Queue\Contracts\IConsumer;
use Yangyao\Queue\Consumer\Proc\Thread;
use Yangyao\Queue\Queue;
use Exception;
class Proc implements IConsumer{
    ...
    public function exec($queue_name,$max,$phpExec=''){
        ...
        while(1) {
            //执行死循环
            try {
                while ($this->threadRunning < $max && !Queue::instance()->is_end($queue_name)) {
                    $this->running[] = new Thread($queue_name,$phpExec);
                    usleep(200000);
                    $this->threadRunning++;
                }
            }
                ...
                switch($e->getCode()) {
                case 30001:
                    logger::emergency(sprintf('ERROR:%d @ %s', $e->getCode(), $e->getMessage));
                    exit;
                }
            }
